...
ImageReader::ImageReader(ImageFormat* res, enum AIMAGE_FORMATS format)
    : reader_(nullptr), presentRotation_(0), imageHeight_(res->height), imageWidth_(res->width), buffer_count(0) {
    media_status_t status = AImageReader_new(res->width, res->height, format, MAX_BUF_COUNT, &reader_);
...
void ImageReader::SetPresentRotation(int32_t angle) { presentRotation_ = angle; }
